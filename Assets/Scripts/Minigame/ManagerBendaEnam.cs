...
public class ManagerBendaEnam : MonoBehaviour
{
    public GameObject BendaSatu, BendaDua, BendaTiga, BendaEmpat, BendaLima,BendaEnam, BendaSatuHitam, BendaDuaHitam, BendaTigaHitam, BendaEmpatHitam, BendaLimaHitam,BendaEnamHitam, finish;

    Vector2 BendaSatuInitialPos, BendaDuaInitialPos, BendaTigaInitialPos, BendaEmpatInitialPos, BendaLimaInitialPos, BendaEnamInitialPos;

    public AudioSource source;
    public AudioClip[] correct;
    public AudioClip incorrect;

    bool BendaSatuCorrect, BendaDuaCorrect, BendaTigaCorrect, BendaEmpatCorrect, BendaLimaCorrect, BendaEnamCorrect = false;
    ...
    void Start()
    {
        ...
        BendaEnamInitialPos = BendaEnam.transform.position; 
    }
    ...
    public void DragBendaEnam()
    {
        BendaEnam.transform.position = Input.mousePosition;
    }
    ...
    public void DropBendaEnam()
    {
        float Distance = Vector3.Distance(BendaEnam.transform.position, BendaEnamHitam.transform.position);
        if (Distance < 50)
        {
            BendaEnam.transform.position = BendaEnamHitam.transform.position;
            source.clip = correct[Random.Range(0, correct.Length)];
            source.Play();
            BendaEnamCorrect = true;
        }
        else
        {
            BendaEnam.transform.position = BendaEnamInitialPos;
            source.clip = incorrect;
            source.Play();
        }
    }
    ...
    void Update()
    {
        if(BendaSatuCorrect && BendaDuaCorrect && BendaTigaCorrect && BendaEmpatCorrect && BendaLimaCorrect && BendaEnamCorrect)
        {
